...
from ilex.plot import _plot_err, plot_PA, plot_stokes
import yaml
import numpy as np
...
def _PA_multi(args):
    ...
    PLOT_ERR_TYPE = pars['plots']['plot_err_type']
    ...
    for i, tcrop in enumerate(tcrops):
        ...
        PA_i, PAerr_i = calc_PAdebiased(data_i, Ldebias_threshold = pars['plots']['debias_threshold'])
    ...
    plot_stokes(data_full, stk_type = "t", ax = AX_PA['S'], Ldebias = pars['plots']['Ldebias'],
            debias_threshold = pars['plots']['debias_threshold'], plot_err_type = PLOT_ERR_TYPE,
            plot_L = pars['plots']['plot_L'])
    ...
    if args.RMplots:
        ...
        for i in range(ncomps):
            _plot_err(ax = AX_RM[i], x = RMcomp_fits[i].x, y = RMcomp_fits[i].y, 
                err = RMcomp_fits[i].yerr, plot_type = frb.plot_err_type, col = 'k')
    ...
    if args.RMburst:
        ...
        _plot_err(ax = ax_B, x = data_full['time'], y = data_full['tI'], 
            err = data_full['tIerr'], plot_type = frb.plot_err_type, col = 'k')
